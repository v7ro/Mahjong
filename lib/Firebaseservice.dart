...
class FirebaseService {
  ...
  Future<void> saveScore(int score) async {
    ...
    // Если очки 0 — не сохраняем (незачем засорять рейтинг)
    if (score <= 0) return;
    ...
    // Всегда обновляем лучший счёт если текущий выше
    if (score > prev) {
      await ref.set({
    ...
    // Месячный рейтинг — обновляем лучший результат за месяц
    final mk = _monthKey();
    final mr = _db.collection('scores_monthly').doc(mk).collection('users').doc(u.uid);
    final ms = await mr.get();
    final prevMonthly = (ms.data()?['score'] ?? 0) as int;

    if (score > prevMonthly) {
      // Новый рекорд за месяц — сохраняем
      await mr.set({
        'score': score,
        'name': displayName,
      ...
      // Не рекорд, но убеждаемся что запись в месячном рейтинге вообще есть
      // (нужно для новых игроков у которых первая игра не стала рекордом)
      if (!ms.exists) {
        await mr.set({
          'score': score,
          'name': displayName,
